...
const mobileNavigation = readFileSync("src/components/project-bottom-navigation.tsx", "utf8");
...
describe("Poultry 2.0 integrity contracts", () => {
  ...
  it("shows Back and an explicit exit to projects on nested pages", () => { expect(routeNavigation).toContain("← Назад"); expect(routeNavigation).toContain("if (!nested) return null"); expect(routeNavigation).toContain("FEVORA · Проекты"); });
  ...
  it("uses the five required contextual navigation destinations", () => { for (const label of ["Обзор", "Птица", "Добавить", "Корм", "Аналитика"]) expect(mobileNavigation).toContain(`label: "${label}"`); });
  it("offers all nine Poultry actions in a mobile sheet", () => { for (const label of ["Яйца", "Корм", "Инкубацию", "Птицу", "Перевод", "Падёж", "Забой", "Продажу", "Расход"]) expect(mobileNavigation).toContain(`label: "${label}"`); });
  it("supports feed use by purpose without requiring a batch", () => { expect(schema).toContain("purpose       String?"); expect(feedPage).toContain("Птица на откорм"); expect(feedPage).toContain("Несушки"); });
  it("lets the user set an exact physical feed balance without creating money", () => { expect(feedPage).toContain("Сколько есть сейчас, кг"); expect(feedPage).toContain("Денежный расход и новая покупка не создаются"); });
  it("allows unused feed types to be deleted while protecting feed history", () => { expect(server).toContain("export async function deleteUnusedFeedProduct"); expect(server).toContain("используется в истории и не может быть удалён"); });
  it("stores the optional egg collector", () => { expect(schema).toContain("collector     String?"); expect(actions).toContain('formValue(formData, "collector")'); });
