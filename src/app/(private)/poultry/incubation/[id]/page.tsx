...
export default async function IncubationBatchPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const returnTo = `/poultry/incubation/${batch.id}`;
  ...
  return <>
    ...
    {batch.items.length ? <div className="transaction-list">{batch.items.map((item) => <AppCard key={item.id}><div className="card-title"><h2>{item.breed || "Порода неизвестна"}</h2><StatusBadge tone="neutral">Вывод {item.hatchRate}%</StatusBadge></div><p className="muted">Заложено {item.setQuantity} · неоплод {item.infertileQuantity} · потери {item.lossQuantity} · вылупилось {item.hatchedQuantity}</p><form action={updateIncubationItemAction} className="compact-form"><input type="hidden" name="itemId" value={item.id}/><input type="hidden" name="returnTo" value={returnTo}/><label>Фактическая дата вывода<input name="hatchDate" type="date" required defaultValue={batch.hatchDate?.toISOString().slice(0,10) ?? ""}/></label><div className="form-grid"><label>Неоплод<input name="infertileQuantity" inputMode="numeric" defaultValue={item.infertileQuantity}/></label><label>Потери<input name="lossQuantity" inputMode="numeric" defaultValue={item.lossQuantity}/></label></div><label>Вылупилось<input name="hatchedQuantity" inputMode="numeric" defaultValue={item.hatchedQuantity}/></label><PoultryFormActions cancelHref="/poultry/incubation"><button className="button secondary">Сохранить результат</button></PoultryFormActions></form>{item.hatchedQuantity > 0 && batch.hatchDate && !item.poultryBatch && <form action={createBatchFromIncubationAction} className="compact-form"><input type="hidden" name="itemId" value={item.id}/><input type="hidden" name="returnTo" value={returnTo}/><PoultryFormActions cancelHref={returnTo}><button className="button primary">Создать группу из вывода</button></PoultryFormActions></form>}{item.poultryBatch && <Link href={`/poultry/flock/${item.poultryBatch.id}`} className="text-link">Открыть созданную группу →</Link>}</AppCard>)}</div> : <EmptyState title="Нет данных по породам" description="В этой закладке пока нет строк." />}
  </>;
}
