...
export default async function PoultryEggsPage({ searchParams }: { searchParams: Promise<{ batchId?: string; returnTo?: string; all?: string }> }) {
  ...
  return <>
    ...
    <div className="page-actions prominent-actions"><Link href="/poultry/eggs?returnTo=%2Fpoultry%2Feggs#collect" className="button primary">Записать сбор</Link><Link href="/poultry/sales?returnTo=%2Fpoultry%2Feggs&type=EGGS#new-sale" className="button secondary">Продать яйца</Link></div>
    <details id="collect" className="app-card action-drawer"><summary><b>Новый сбор</b><span>＋</span></summary><form action={createEggCollectionAction} className="compact-form"><input type="hidden" name="returnTo" value={returnTo}/><div className="form-grid"><label>Количество<input name="quantity" required inputMode="numeric" autoFocus/></label><label>Дата<input name="operationDate" type="date" required defaultValue={today}/></label></div><details className="optional-details"><summary>Добавить детали</summary><label>Группа<select name="batchId" defaultValue={selectedBatchId}><option value="">Общий сбор / неизвестно</option>{batches.filter((batch) => batch.status === "ACTIVE").map((batch) => <option value={batch.id} key={batch.id}>{batch.name}</option>)}</select></label><div className="form-grid"><label>Порода<input name="breed" placeholder="Необязательно"/></label><label>Кто собрал<input name="collector" placeholder="Необязательно"/></label></div><label>Комментарий<input name="comment"/></label></details><PoultryFormActions cancelHref={returnTo}><button className="button primary">Сохранить сбор</button></PoultryFormActions></form></details>
