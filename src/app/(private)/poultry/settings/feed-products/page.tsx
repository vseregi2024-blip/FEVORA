...
export default async function FeedProductsPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  return <>
    ...
    <div className="settings-list">{products.map((product) => {
      ...
      return <details className="settings-editor inline-editor" key={product.id}><summary><span><b>{product.name}</b><small>{product.type || "Без категории"}{used ? " · есть история" : " · можно удалить"}</small></span><span>Изменить</span></summary><form action={updateFeedProductCatalogAction} className="compact-form"><input type="hidden" name="id" value={product.id}/><input type="hidden" name="returnTo" value="/poultry/settings/feed-products"/><label>Название<input name="name" required defaultValue={product.name}/></label><label>Категория<select name="type" defaultValue={product.type ?? "Комбикорм"}><option>Зерно</option><option>Комбикорм</option><option>Добавки</option></select></label><PoultryFormActions cancelHref="/poultry/settings/feed-products"><button className="button primary">Сохранить</button></PoultryFormActions></form>{!used && <form action={deleteFeedProductAction} className="inline-settings-actions"><input type="hidden" name="id" value={product.id}/><button className="text-button danger-text">Удалить вид корма</button></form>}</details>;
    })}</div>
