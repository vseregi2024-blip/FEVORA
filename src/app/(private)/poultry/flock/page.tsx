...
export default async function FlockPage() {
  ...
  return <>
    <header className="page-header"><div><p className="eyebrow">Птицеводство</p><h1>Поголовье</h1><p className="muted">Учёт ведётся по партиям, а не общим числом.</p></div><Link href="/poultry" className="button secondary">Назад</Link></header>
    ...
    <SectionHeader title="Партии" />
    <section className="project-grid">{batches.map((batch) => <Link className="project-card active" href={`/poultry/flock/${batch.id}`} key={batch.id}><div className="card-title"><h3>{batch.name}</h3><StatusBadge tone={batch.status === "ACTIVE" ? "success" : "neutral"}>{batch.status === "ACTIVE" ? "Активна" : "Завершена"}</StatusBadge></div><span>{batch.birdType}{batch.breed ? ` · ${batch.breed}` : ""}</span><strong>{batch.currentQuantity} гол.</strong><small>Старт: {batch.startingQuantity} · возраст: {batch.ageDays} дн.</small><em>Открыть партию →</em></Link>)}</section>
    {batches.length === 0 && <EmptyState title="Пока нет партий птицы" description="Создайте первую партию, чтобы учитывать поголовье и связанные операции." />}
  </>;
}
