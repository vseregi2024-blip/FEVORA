...
export default async function FeedProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <>
    <header className="page-header compact-page-header"><div><p className="eyebrow">Склад кормов</p><h1>{item.product.name}</h1><p className="muted">{item.product.type || "Без категории"}</p></div></header>
