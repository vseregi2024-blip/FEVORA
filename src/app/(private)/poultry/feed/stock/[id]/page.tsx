...
import { createFeedAdjustmentAction, updateFeedProductSettingsAction } from "../../../actions";
...
export default async function FeedProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <>
    ...
    <div className="action-button-grid"><Link href="#adjust-stock" className="button primary">Изменить остаток</Link><Link href="/poultry/feed?returnTo=%2Fpoultry%2Ffeed%2Fstock#buy-feed" className="button secondary">Добавить покупку</Link><Link href="/poultry/feed?returnTo=%2Fpoultry%2Ffeed%2Fstock#assign-feed" className="button secondary">Использовать</Link></div>
    <details id="adjust-stock" className="app-card action-drawer"><summary><b>Фактический остаток</b><span>＋</span></summary><form action={createFeedAdjustmentAction} className="compact-form"><input type="hidden" name="productId" value={id}/><input type="hidden" name="returnTo" value={`/poultry/feed/stock/${id}`}/><label>Сколько есть сейчас, кг<input name="actualKg" required inputMode="decimal" defaultValue={item.current.toFixed(1)}/></label><label>Дата сверки<input name="operationDate" type="date" required defaultValue={todayInputValue()}/></label><label>Комментарий<input name="comment" placeholder="Необязательно"/></label><p className="summary-note">Укажите фактическое количество. FEVORA сохранит только разницу и не создаст новый расход денег.</p><PoultryFormActions cancelHref={`/poultry/feed/stock/${id}`}><button className="button primary">Сохранить остаток</button></PoultryFormActions></form></details>
