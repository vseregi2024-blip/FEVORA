...
export default async function FeedStockPage({ searchParams }: { searchParams: Promise<{ category?: string; status?: string }> }) {
  ...
  return <>
    <header className="page-header compact-page-header"><div><p className="eyebrow">Корм</p><h1>Склад кормов</h1><p className="muted">Нажмите на корм, чтобы изменить фактический остаток</p></div><div className="page-actions"><Link href="/poultry/feed?returnTo=%2Fpoultry%2Ffeed%2Fstock#feed-reconcile" className="button primary">Сверить остаток</Link><Link href="/poultry/settings/feed-products" className="button secondary">Виды кормов</Link></div></header>
