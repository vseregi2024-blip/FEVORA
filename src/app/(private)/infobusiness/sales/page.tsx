...
export default async function InfoSalesPage() {
  ...
  return <><header className="page-header"><div><p className="eyebrow">Инфобизнес · Продажи</p><h1>Продажи обучения</h1><p className="muted">Одна продажа создаёт ровно один доход.</p></div><Link href="/infobusiness" className="button secondary">Назад</Link></header>
    ...
    <section className="section-header"><div><p className="eyebrow">История</p><h2>Все продажи</h2></div></section><div className="transaction-list">{sales.map((sale) => <Link className="transaction-row" href={`/infobusiness/sales/${sale.id}`} key={sale.id}><span><b>{sale.product.name}</b><small>{sale.incomeTransaction.operationDate.toLocaleDateString("ru-RU")}{sale.buyer ? ` · ${sale.buyer}` : ""}{sale.seats !== 1 ? ` · ${sale.seats} мест` : ""}</small></span><strong className="income">+{formatMoney(sale.incomeTransaction.amount.toString())}</strong></Link>)}</div>{sales.length === 0 && <EmptyState title="Продаж пока нет" description="Добавьте продажу обучения — она сразу попадёт в доход Инфобизнеса."/>}</>;
}
