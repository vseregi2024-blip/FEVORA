...
export default async function InfoSalePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <><header className="page-header"><div><p className="eyebrow">Инфобизнес · Продажа</p><h1>Редактировать продажу</h1></div><Link href="/infobusiness/sales" className="text-link">К продажам</Link></header><InfoSaleForm products={dashboard.products.filter((product) => product.status !== "ARCHIVED" || product.id === sale.productId).map((product) => ({ id: product.id, name: product.name, basePrice: product.basePrice?.toString() ?? null }))} action={updateInfoSaleAction} initialValues={{ id: sale.id, productId: sale.productId, amount: sale.incomeTransaction.amount.toString(), operationDate: dateToInput(sale.incomeTransaction.operationDate), buyer: sale.buyer ?? "", seats: sale.seats, comment: sale.comment ?? "" }}/><div className="danger-zone"><h2>Удаление</h2><p>Связанный Income также будет исключён из отчётов.</p><DeleteInfoBusinessRecordForm id={sale.id} action={deleteInfoSaleAction} kind="sale"/></div></>;
}
