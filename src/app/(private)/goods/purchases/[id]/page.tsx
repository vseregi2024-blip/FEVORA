...
export default async function PurchasePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <>
    ...
    {locked ? <p className="summary-note">Эта партия уже использована. Данные защищены от изменений, чтобы не исказить себестоимость уже сделанных продаж.</p> : <form action={updatePurchaseAction} className="transaction-form"><input type="hidden" name="id" value={purchase.id}/>
      ...
      <div className="form-grid"><label>Количество<input name="quantity" required inputMode="numeric" defaultValue={purchase.quantity}/></label><label>Закупочная цена за единицу<input name="unitPurchasePrice" required inputMode="decimal" defaultValue={purchase.unitPurchasePrice.toString()}/></label></div>
