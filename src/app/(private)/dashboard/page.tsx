...
const moduleNames = { FAMILY: "Семья", POULTRY: "Птицеводство", GENERAL: "Общее", GOODS: "Товарка", INFOBUSINESS: "Инфобизнес" } as const;
export default async function DashboardPage() {
  const user = await requireUser(); const today = todayInputValue(); const monthStart = `${today.slice(0, 7)}-01`;
  const [allTime, month, recent, goods, infobusiness] = await Promise.all([getFinancialSummary(user.id), getFinancialSummary(user.id, dateFromInput(monthStart), dateFromInput(today)), getTransactions(user.id), getGoodsDashboard(user.id, "MONTH"), getInfoDashboard(user.id, "MONTH")]);
