...
const moduleLabels: Record<Exclude<FinanceModule, "GOODS">, string> = { GENERAL: "Общее", FAMILY: "Семья", POULTRY: "Птицеводство" };
...
export function TransactionForm({ categories, initialValues, action, submitLabel }: { categories: Category[]; initialValues: InitialValues; action: (formData: FormData) => void | Promise<void>; submitLabel: string }) {
  const [module, setModule] = useState<Exclude<FinanceModule, "GOODS">>(initialValues.module === "GOODS" ? "GENERAL" : initialValues.module);
