...
const moduleLabels: Record<Exclude<FinanceModule, "GOODS" | "INFOBUSINESS" | "COSMETOLOGY">, string> = { GENERAL: "Общее", FAMILY: "Семья", POULTRY: "Птицеводство" };
...
export function TransactionForm({ categories, initialValues, action, submitLabel }: { categories: Category[]; initialValues: InitialValues; action: (formData: FormData) => void | Promise<void>; submitLabel: string }) {
  const [module, setModule] = useState<Exclude<FinanceModule, "GOODS" | "INFOBUSINESS" | "COSMETOLOGY">>(initialValues.module === "GOODS" || initialValues.module === "INFOBUSINESS" || initialValues.module === "COSMETOLOGY" ? "GENERAL" : initialValues.module);
